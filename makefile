...
build: format get
	CGO_ENABLED=0 GOOS=${TARGETOS} GOARCH=${TARGETARCH} go build -v -o kbot-${TARGETOS}-${TARGETARCH} -ldflags "-X="github.com/alex-moiseenko/kbot/cmd.appVersion=${VERSION}
...
image-macos: image-darwin-amd64 image-darwin-arm64

image-linux: image-linux-amd64 image-linux-arm64
...
clean:
	...
	-docker ps -a -q --filter ancestor=${REGESTRY}/${APP}:${VERSION}-${TARGETARCH} | xargs -r docker rm -f
	-docker rmi -f ${REGESTRY}/${APP}:${VERSION}-${TARGETARCH} || true